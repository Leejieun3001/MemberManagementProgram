...
class ReservationPage(Frame):
    ...
    def applyCancel(self, n):
        ...
        #새로 예약하는 경우 
            ...
            if isReserve:
                sql = "insert into mmp.sugan  (mnum, cnum) values ("+str(self.userNum) +","+ classNum +");"
                curs.execute(sql)
                conn.commit()
                self.refresh()
                self.controller.show_frame("ReservationPage")
...
class MyPage(Frame):
    ...
    def __init__(self, parent, controller):
        ...
        # SQL문 실행
        sql = "select mname from member where mnum = %s"
    ...
    def mypage(self):
        ...
        sql = '''select c.cname, c.ccontent, c.ctime, count(s.cnum) nowNum, c.cmax from class c inner
                 join sugan s on c.cnum = s.cnum where c.cnum = any(select s.cnum from sugan s where s.mnum = %s)
