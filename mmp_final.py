...
class ReservationPage(Frame):
    ...
    def applyCancel(self, n):
        ...
        if len(rows) :            
            sql ="select * from class where cnum ="+classNum+" and CAST(strftime('%s', ctime) AS integer ) > CAST(strftime('%s', datetime('now','localtime')) AS integer ) and CAST(strftime('%s', ctime) AS integer ) < CAST(strftime('%s', datetime('now','localtime','+24 hours')) AS integer)"
            curs2 = conn.cursor()
            curs2.execute(sql)
            rows2 = curs2.fetchall()
            curs2.close()
            if len(rows2):
                messagebox.showinfo("취소 불가","24시간 이내의 수업은 취소 할 수 없습니다.")
            else:
...
class LoginPage(Tk):
    ...
    def __init__(self):
        ...
        #테이블 생성
        sql1=  "CREATE TABLE IF NOT EXISTS sugan (    snum INTEGER PRIMARY KEY AUTOINCREMENT, mnum INTEGER,   cnum	INTEGER,	FOREIGN KEY(cnum) REFERENCES class (cnum));"
        sql2 = "CREATE TABLE IF NOT EXISTS member (mnum INTEGER PRIMARY KEY AUTOINCREMENT, mname	TEXT, mphone INTEGER);"
        sql3 = "CREATE TABLE IF NOT EXISTS class (cnum	INTEGER PRIMARY KEY AUTOINCREMENT, cname TEXT, ctime TEXT, cmax INTEGER, ccontent	TEXT);"
