...
class ReservationPage(Frame):
    ...
    def update(self):
        ...
        if(UPDATE!=0) :
            ...
            self.classes[self.i].grid(row=3, column=0)
            #db 연결
            conn = sqlite3.connect("mmp.db")
    ...
    def addClass(self):
        ...
        if (messagebox.askokcancel("Redirecting", "등록하시겠습니까?")):
            ...
            ###수업등록
                ...
                try :
                    global UPDATE
                    date = str(self.cDate1.get()) + "-" + str(self.cDate2.get()).zfill(2) + "-" + str(self.cDate3.get()).zfill(2) + " " + str(self.cDate4.get()).zfill(2) + ":00:00"
